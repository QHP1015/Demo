...
module.exports = app => {
    ...
    const Article = mongoose.model('Article', schema1)
    const Category = mongoose.model('Category', schema2)
    const Hero = mongoose.model('Hero', schema3)
    ...
    router.get('/heroes/init', async (req, res) => {
        ...
        for (let cat of rawData.filter(item => item.name !== '热门')) {
            // if(cat.name === '热门'){
            //     continue
            // }
